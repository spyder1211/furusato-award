        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-3">
                <label for="role_municipality"
                       class="relative flex items-center justify-center p-4 border-2 rounded-lg cursor-pointer transition-all duration-200 {{ old('role', request('role')) === 'municipality' ? 'border-primary-600 bg-primary-50' : 'border-gray-300 hover:border-primary-300' }}">
                    <input id="role_municipality"
                           type="radio"
                           name="role"
                           value="municipality"
                           class="sr-only"
                           {{ old('role', request('role')) === 'municipality' ? 'checked' : '' }}
                           required>
                    <div class="text-center">
                        <svg class="w-8 h-8 mx-auto mb-2 {{ old('role', request('role')) === 'municipality' ? 'text-primary-600' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                        </svg>
                        <span class="text-sm font-semibold {{ old('role', request('role')) === 'municipality' ? 'text-primary-600' : 'text-gray-700' }}">首長（自治体）</span>
                    </div>
                </label>

                <label for="role_company"
                       class="relative flex items-center justify-center p-4 border-2 rounded-lg cursor-pointer transition-all duration-200 {{ old('role', request('role')) === 'company' ? 'border-secondary-600 bg-secondary-50' : 'border-gray-300 hover:border-secondary-300' }}">
                    <input id="role_company"
                           type="radio"
                           name="role"
                           value="company"
                           class="sr-only"
                           {{ old('role', request('role')) === 'company' ? 'checked' : '' }}
                           required>
                    <div class="text-center">
                        <svg class="w-8 h-8 mx-auto mb-2 {{ old('role', request('role')) === 'company' ? 'text-secondary-600' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                        </svg>
                        <span class="text-sm font-semibold {{ old('role', request('role')) === 'company' ? 'text-secondary-600' : 'text-gray-700' }}">企業</span>
                    </div>
                </label>
            </div>
